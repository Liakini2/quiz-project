module.exports = {
    ...
    addQuiz: async (req, res)=>{
        const {quiz_image, description, title} = req.body
        const {user_id} = req.session.user
        const db = req.app.get('db')
        ...
        const [result] = await db.quiz.add_quiz([user_id, quiz_image, description, title])
        const newQuiz = {
    ...
    editQuiz: async (req, res)=>{
        ...
        if(result){
            const [updatedQuiz] = await db.quiz.edit_quiz([quiz_id, user_id, quiz_image?quiz_image:result.quiz_image, description?description:result.description, title?title:result.title])
